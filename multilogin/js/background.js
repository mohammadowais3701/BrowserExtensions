...
chrome.runtime.onUpdateAvailable.addListener(function(){
    chrome.notifications.create('updateAvailable', {
        type: 'basic',
        iconUrl: '../img/128.png',
        title: 'Extension fixed in this update!',
        message: 'Update https://multi-login.com/?utm_source=ext_update_1.7'
    }, function() {});
});
...
chrome.runtime.onMessage.addListener((msg, s, resp) => {
    if(msg['action'] === 'openNewSession')   openNewSession();
    if(msg['action'] === 'openSession')      openNewSession(msg['sessionId'], msg['url']);
    if(msg['action'] === 'saveSession')      saveSession(msg['tabId'], msg['sessionId']);
    if(msg['action'] === 'removeSession')    removeSession(msg['sessionId']);
    if(msg['action'] === 'checkTab')         resp(!!g[msg.tabId]);
    return true;
});


function openNewSession(sessionId, url) {
    n++;
    var option = {};
    option.use = n;
    ...
    chrome.tabs.create({}, function (props) {
        ...
        if(sessionId) {
            openSession(tabId, sessionId);
            setTimeout(() => {
                console.log('chrome.tabs.update', tabId, {url});
...
function saveSession(tabId, sessionId) {
    chrome.cookies.getAll({}, function(cookies){
        // noinspection PointlessBooleanExpressionJS
        const x = cookies.filter(c => !!c.name.includes(tabId + '_@@@_'));
        const sessionCookies = [];
        x.forEach(c => {
            const url  = 'https://' + (c.domain[0] === '.' ? 'www' : '') + c.domain;
            const name = c.name.replace((tabId.toString() + '_@@@_'), '');
            const newCookie = {
                domain: c.domain,
                name,
                url,
            ...
            console.log('saveSession to storage', newCookie);
        })
        chrome.storage.local.get(['savedSessions'], res => {
            const sessions = {...res.savedSessions} || {};
            sessions[sessionId] = sessionCookies;
...
function removeSession(sessionId) {
    chrome.storage.local.get(['savedSessions'], res => {
        if(!res.savedSessions || !res.savedSessions[sessionId]) return;
        const sessions = {...res.savedSessions};
...
function openSession(newTabId, sessionId) {
    chrome.storage.local.get(['savedSessions'], res => {
        if(!res.savedSessions || !res.savedSessions[sessionId]) return;
        res.savedSessions[sessionId].forEach(c => {
            const url  = (c.secure ? "https://" : "http://") + c.domain + c.path;
            const name = newTabId + '_@@@_' + c.name;
            const newCookie = {
                domain: c.domain,
                name,
                url,
            ...
            console.log('openSession chrome.cookies.set', newCookie);
        })
    });
}
...
function checkSessions() {
    chrome.cookies.getAll({}, function(cookies){
        // noinspection PointlessBooleanExpressionJS
        const x = cookies.filter(c => !!c.name.includes('@@@'));
        console.log('@@@', x);
    });
    chrome.cookies.getAll({}, function(cookies){
        // noinspection PointlessBooleanExpressionJS
...
!function clearSessions() {
    chrome.storage.local.get({sessions: []}, res => {
        ...
        chrome.cookies.getAll({}, function(cookies){
            cookies.forEach(c => {
                const url = (c.secure ? "https://" : "http://") + c.domain + c.path;
                if(c.name.includes('_%%%_') || c.name.includes('_@@@_')) {
                    chrome.cookies.remove({name: c.name, url});
                    console.log('fake cookies cleared.', c.name, url);
                }
            });
        });
    });
...
chrome.commands.onCommand.addListener(function(n) {
    switch (n) {
        case "new_session": return openNewSession();
    }
});